...
int CheckResponseHeader(DNS_HEADER* dnsHeader)
{
	...
	{
		if (responseCode == 0x0001) { errorCode = DNS_BAD_FORMAT; }
		else if (responseCode == 0x0002) { errorCode = DNS_SERVER_FAIL; }
		else if (responseCode == 0x0003) { errorCode = DNS_NON_EXIST; }
		else if (responseCode == 0x0004) { errorCode = DNS_NOT_SUPPORT; }
		else if (responseCode == 0x0005) { errorCode = DNS_REFUSED; }
		else { errorCode = DNS_BAD_RCODE; }
		return INVALID;
	}
